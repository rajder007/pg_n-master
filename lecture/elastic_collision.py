...
def check_ball_collision(ball_red, ball_blasck):
    ...
    if distance < red_ball.ball_radius + black_ball.ball_radius:
        # Calculate new velocities after collision (in Zero-G mechanics)
        normal = [red_ball.ball_pos.x - black_ball.ball_pos.x, 
                  red_ball.ball_pos.y - black_ball.ball_pos.y]
        norm = math.sqrt(normal[0]**2 + normal[1]**2)
        unit_normal = [normal[0] / norm, normal[1] / norm]

        v1 = red_ball.ball_speed
        v2 = black_ball.ball_speed
        v1n = [v1[0] * unit_normal[0] + v1[1] * unit_normal[1], v1[0] * unit_normal[1] - v1[1] * unit_normal[0]]
        v2n = [v2[0] * unit_normal[0] + v2[1] * unit_normal[1], v2[0] * unit_normal[1] - v2[1] * unit_normal[0]]

        v1f = [v2n[0] * unit_normal[0] - v1n[1] * unit_normal[1], v1n[0] * unit_normal[1] + v2n[1] * unit_normal[0]]
        v2f = [v1n[0] * unit_normal[0] - v2n[1] * unit_normal[1], v2n[0] * unit_normal[1] + v1n[1] * unit_normal[0]]

        red_ball.ball_speed =  pygame.math.Vector2(v1f)
        black_ball.ball_speed =pygame.math.Vector2(v2f)
